...
public class Expression {
	...
	private String simplify()// 表达式是getExp(),命令是getCmd()
	{
		...
		/*
		 * for (int i = 0; i < getExp().length(); i++) { char c =
		 * getExp().charAt(i); if (!((c >= '0' && c <= '9') || (c >= 'a' && c <=
		 * 'z') || c == '+' || c == '*' || c == '-')) { setExp(null);
		 * System.out.println("Please check the input!"); stringsum =
		 * "your string contains illegal charactor"; return stringsum; } }
		 */
		if (getCmd() == "!simplify") {
			stringsum = eq;
			return stringsum;
